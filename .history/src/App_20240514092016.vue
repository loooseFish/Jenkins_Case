<template>
  <div id="app">
    <home />
    <button @click="$msg.show({ type: 'success', content: 'hello' });">success</button>
    <button @click="$msg.show({ type: 'primany' });">primany</button>
    <button @click="$msg.show({ type: 'error' });">error</button>
    <button @click="$msg.show({ type: 'info' });">info</button>
    <!--  -->
    ||
    <button @click="add">add</button>
    <button @click="del">del</button>
    {{ set }}
    ||
  </div>
</template>

<script>
import home from './views/home.vue'

export default {
  data() {
    return {
      number: [1, 2, 3, 4, 5, 6],
      set: {

      }
    }
  },
  components: {
    home
  },
  methods: {
    add() {
      if (!this.set.addd) {
        this.$set(this.set, 'addd', 1)
      } else {
        this.set.addd++
      }
    },
    del() {
      this.$delete(this.set, 'addd')
    }
  },
  created() {
    // new
    {
      // new -----------------------------------------------------------------------------

      // new 操作符做了什么事情
      // function Foo() {

      // }

      // prototype 和 __proto__ (yes)
      // console.log(Foo.prototype === new Foo().__proto__);

      // new 模拟执行
      function Fun(age, name) {
        this.age = age;
        this.name = name;
      }

      function create(fn, ...args) {
        // 创建空对象
        var obj = {};
        // 将空对象的原型，指向与构造函数的原型
        // 将空对象作为构造函数上下文（改变this指向
        var result = Object.setPrototypeOf(fn.apply(obj, args), fn.prototype);
        // 判断返回类型
        return result instanceof Object ? result : obj
      }

      console.log(create(Fun, 18, '张三'));

      // new -----------------------------------------------------------------------------
    }
    // call,apply,bind
    {
      // call,apply,bind -----------------------------------------------------------------------------

      // function fun() {
      //   console.log('hello world');
      //   console.log(this);
      // };
      // 1.call可以调用方法
      // 2.call可以更改this指向
      // let dog = {
      //   name: '旺财',
      //   sayName(food1 = 1, food2 = 2) {
      //     console.log("dogName:" + this.name + food1 + food2);
      //   },
      //   eat(food1, food2) {
      //     console.log("dogEat:" + food1 + food2);
      //   }
      // }
      // let cat = {
      //   name: '喵喵'
      // }
      // fun.call(cat)
      // dog.sayName.call(cat)
      // 3.call可以传参
      // dog.sayName.call(cat, '鱼', 'meme')
      // apply
      // dog.eat.apply(cat, ['鱼', 'meme'])
      // bind
      // dog.eat.bind(cat, '鱼', 'meme')()
      // console.log();

      // function Aminal() {
      //   this.eat = function () {
      //     console.log('eat', this);
      //   }
      // }

      // function bird() {
      //   this.fly = function () {
      //     console.log('fly');
      //   }
      // }

      // function Cat() {
      //   console.log(this);

      //   Aminal.call(this);
      //   bird.call(this);
      // }

      // let cats = new Cat();
      // cats.fly();
      // cats.eat();

      // new Aminal().eat();
      // Aminal.call(dog)

      // call,apply,bind -----------------------------------------------------------------------------
    }
    // 原型、原型链
    {
      // 原型链、原型
      function pro(params) {
        this.a = 1
      }
      // console.dir(pro)
      // console.log(pro.prototype.constructor);
      // console.log(new pro().__proto__);
      // console.log(pro.prototype.__proto__);
      // console.log(Object.prototype);

      // console.log(pro.prototype === new pro().__proto__);
      // console.log(pro.prototype.__proto__ === Object.prototype);

      // pro.prototype.b = 222;
      // pro.prototype.__proto__.c = 333;
      // // console.log(new pro().b, new pro().c);
      // let Pro = new pro();
      // // 判断非原型继承属性是否存在
      // console.log(Pro.hasOwnProperty('a'));
      // console.log(Pro.hasOwnProperty('b'));
      // // 找原型链上是否有继承属性
      // console.log('a' in Pro)
      // console.log('b' in Pro)
    }

  },
  render(createEle) {
    return createEle('ul', {
      attrs: { class: 'caseUl' }, on: {
        click: () => {
          console.log('render元素: ul');
        }
      }
    }, this.number.map(item => createEle('li', { attrs: { class: 'caseLi' } }, `${item}`)))
  },

}

</script>

<style>
.caseUl {
  border: 1px solid #000;
}

.caseLi {
  border: 1px solid #d21e1e;
}
</style>